...
- Server on the Mac: `python3 build/yearround/api.py` → http://192.168.113.105:8787
...
- Check from the phone first: open http://192.168.113.105:8787/v1/health in Safari. If it fails,
...
1. Shortcuts → **+** new shortcut, name it **Tap n' tax**. Actions, in order:
   1. **Get Contents of URL** — `http://192.168.113.105:8787/v1/events/transaction`, Method POST,
   ...
   3. **Open URLs** — `http://192.168.113.105:8787/`
...
3. Safari: open http://192.168.113.105:8787/ once, **Share → Add to Home Screen** (name "Taxfix")
   so it opens full screen without the Safari bar.

## The shot (iPad + phone side by side, phone screen-recorded)
1. iPad: http://192.168.113.105:8787/terminal shows REWE 38,40 €.
...
Fallback inside Safari: open http://192.168.113.105:8787/ on the **Mac** in a phone-sized window
